...
try {
  if (process.env.EMAIL && process.env.EMAIL_PASSWORD) {
    ...
    console.warn('⚠️ WARNING: EMAIL and EMAIL_PASSWORD env vars are not set. Emails will be logged to console instead.')
  }
} catch (err) {
...
const sendOrderConfirmation = async (email, order, items) => {
  try {
    if (!transporter) {
      console.log(`[Email Mock] Order Confirmation to: ${email}`, { order_number: order.order_number, total: order.total })
      return
    }
...
const sendNewsletterConfirmation = async (email) => {
  try {
    if (!transporter) {
      console.log(`[Email Mock] Newsletter Confirmation to: ${email}`)
      return
    }
